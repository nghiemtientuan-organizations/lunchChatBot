...
function previewUrl(textResponse = '') {
    ...
    if (founds && founds.length) {
        ...
        $.ajax({
            url: `${hotServer}/api/url-info?url=${link}`,
            type: 'GET',
            contentType: 'application/json',
            success: function(data, textStatus) {
                if (data != null) {
                    const BotResponse = `<div class='bot-msg'><div class='bot-img-none'/>` +
                        `<a href="${link}" target="_blank"><div class="previewUrl">` +
                        `<img class="previewUrl-image" src="${data.photo}"><span class="previewUrl-text">${data.name}</span>` +
                        `</div></a>` +
                        '</div>';
                    $(BotResponse).appendTo('.chat-area');
                    scrollToBottomOfResults();
                }
                console.log("previewUrl Response: ", data, "\n Status:", textStatus)
            },
            error: function(errorMessage) {
                console.log('previewUrl Error' + errorMessage);
...
function createChatBot(hotServerURL, hostURL, botLogo, title, welcomeMessage, inactiveMsg, theme = "blue") {
    hotServer = hotServerURL;
    host = hostURL;
    botLogoPath = botLogo;
